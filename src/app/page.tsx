  ...
  return (
    <div style={{ background: T.bg, minHeight: '100vh', paddingBottom: '90px', fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div style={{ position: 'fixed', bottom: 15, left: 15, right: 15, background: T.header, display: 'flex', justifyContent: 'space-around', padding: '10px 5px', borderRadius: '20px', border: `1px solid ${T.gold}`, boxShadow: '0 8px 32px rgba(0,0,0,0.3)', zIndex: 1000 }}>
         {[
           { label: 'Game', icon: <Play size={20} fill={T.gold} color={T.gold} />, active: true },
           { label: 'Scores', icon: <Trophy size={20} color={T.gold} />, active: false },
           { label: 'History', icon: <History size={20} color={T.gold} />, active: false },
           { label: 'Wallet', icon: <WalletIcon size={20} color={T.gold} />, active: false },
           { label: 'Profile', icon: <User size={20} color={T.gold} />, active: false },
         ].map((item) => (
           <div key={item.label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', flex: 1, opacity: item.active ? 1 : 0.5, cursor: 'pointer' }}>
             {item.icon}
